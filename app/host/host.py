...
from app.host.client import MCPClient
...
class MCPHost:
    """Host for MCP protocol implementation."""
    ...
    def __init__(self, config: AppConfig, model_service: ModelService):
        ...
        logger.info("Initialized MCP host")
    ...
    async def _initialize_client(self, server_config: MCPServerConfig) -> None:
        ...
        # TODO: Fetch available resources and tools from the server
        # This would involve making requests to the server to discover capabilities
    ...
    def _extract_mcp_requests(self, text: str) -> List[Dict[str, Any]]:
        """Extract MCP requests from model response.
        
        This method is kept for backward compatibility but now uses the utility function.
        
        Args:
            text: Model response text
            
        Returns:
            List of MCP request dictionaries
        """
        _, requests = extract_mcp_requests_from_text(text)
        return requests
